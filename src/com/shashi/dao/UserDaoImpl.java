...
//			ps = conn.prepareStatement("insert into shopping.user values(?,?,?,?,?,?)");
			...
			if(k>0) {
				...
				ml.registrationSuccess(user.getEmailId(), user.getUserName().split(" ")[0]);
			}
...
//		DBUtil.closeConnection(ps);
	...
	public boolean isRegistered(String emailId) {
		...
		try {
			ps = con.prepareStatement("select * from user where emailId=?");
...
//                        System.out.println(status);
	...
	public String getFName(String emailId) {
		...
		try {
			ps = con.prepareStatement("select name from user where emailId=?");
			ps.setString(1, emailId);
	...
	public String getUserAddr(String userId) {
		...
		try {
			ps = con.prepareStatement("select address from user where emailId=?");
